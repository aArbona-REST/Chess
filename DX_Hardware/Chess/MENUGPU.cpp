...
#pragma endregion
	...
	InitalizeobjAsset(&exit, (char *)"exit_text.obj", (wchar_t *)L"white.dds", &menulistposition[EXIT]);
	//
	...
	InitalizeobjAsset(&credits, (char*)"credits_text.obj", (wchar_t*)L"white.dds", &menulistposition[CREDITS]);
	//
...
#pragma region menu nav
	...
	if (input.buttonbuffer[VK_DOWN] == false && input.buttons[VK_DOWN])
	{
		...
		XMStoreFloat4x4(&icon.send_to_ram2.modelPos, XMMatrixTranspose(menulistposition[presenticonselected]));
...
void MENUGPU::InitalizeobjAsset(OBJECT * object, char * mesh, wchar_t * texture, XMMATRIX * m)
{
	...
	object->initobj(MenuVertexShader, sizeof(MenuVertexShader), MenuPixelShader, sizeof(MenuPixelShader), mesh);
	XMStoreFloat4x4(&object->send_to_ram2.modelPos, XMMatrixTranspose(*m));
...
void MENUGPU::InitalizeQuad(OBJECT * object, wchar_t * texture)
{
	...
	object->inittridebug(MenuVertexShader, sizeof(MenuVertexShader), MenuPixelShader, sizeof(MenuPixelShader), quad.groundPlane, quad.vertcount, quad.groundPlaneindex, quad.indexcount);
